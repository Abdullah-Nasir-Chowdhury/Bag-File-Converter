...
class ConversionWorker(QThread):
    ...
    def __init__(self, bag_files, rs_convert_path):
        super().__init__()
    ...
    def extract_data(self, bag_file, ply_output_path, png_output_path, base_progress, file_progress_weight, 
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                self.update_progress.emit(
                    int(progress),
                    f"Extracting {bag_filename} ({int((0.2 + extract_progress) * 100)}%){time_estimate}"
                )
...
class BagConverterApp(QMainWindow):
    ...
    def start_conversion(self):
        ...
        # Start worker thread
        self.worker = ConversionWorker(selected_files, self.rs_convert_path)
